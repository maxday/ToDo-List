	...
	if(isset($_SERVER['HTTP_X_REQUESTED_WITH'])){
		...
		$result = array(
			array(
				...
				'img' => 'http://random.revaxarts.com/50x50.jpg'
			),
			array(
...
/*----------------------------------------------------------------------*/
	...
	function getResultsThemeForest($searchterm){
		...
		foreach($obj->search as $item){
			if(++$i > 10) continue;
			$array = array(
					'href' => $item->url.'?ref=revaxarts',
					'text' => '<strong>'.$item->description.'</strong>',
		...
		//and put a little info at the beginning
		if(!empty($results)){
			array_unshift($results, array(
					'href' => '#',
					'text' => '<strong>The results are from Themeforest. It\'s very easy to adopt it to your own search results!</strong>',
			));
		}
		return $results;
	}
	...
	function getResultsFromWikipedia($searchterm){
		...
		if(!empty($results)){
			array_unshift($results, array(
					'href' => '#',
					'text' => '<strong>The results are from Wikipedia. It\'s very easy to adopt it to your own search results!</strong>',
			));
		}
		return $results;
	}
